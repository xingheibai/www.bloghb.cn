...
<div class="content">
  <div class="title">
	<h3 style="line-height: 1.3">{{$catename}}</h3>
  </div>
	@foreach($arts as $k=>$v)
  <article class="excerpt excerpt-1"><a class="focus" href="{{url('/art/'.$v['art_id'].'')}}" title="{{$v['art_title']}}" target="_blank" ><img class="thumb" data-original="{{$v['art_thumb']}}" src="{{url($v['art_thumb'])}}" alt="{{$v['art_thumb']}}"  style="display: inline;"></a>
	...
	<p class="meta">
	  ...
	  <span class="views"><i class="glyphicon glyphicon-eye-open"></i> {{$v['art_view']}}</span>
		<a class="comment" href="##comment" title="评论" target="_blank" ><i class="glyphicon glyphicon-comment"></i> {{$v['com_view']}}</a></p>
